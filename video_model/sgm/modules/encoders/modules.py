        ...
        for embedder in self.embedders:
            ...
            with embedding_context():
                if hasattr(embedder, "input_key") and (embedder.input_key is not None):
                    ...
                    if embedder.input_key == 'cond_frames':
                        emb2 = embedder(batch['cond_frames_2'])
                        emb3 = embedder(batch['cond_frames_3'])
                        emb4 = embedder(batch['cond_frames_4'])

                        repeat_n = 8               # how many copies of each extra embedding
                        pieces = []

                        for i in range(emb_out.shape[0]):          # loop over the batch dimension B
                            pieces.extend([
                                emb_out[i : i + 1],                # original (1, C, H, W)
                                emb2[i : i + 1].repeat(repeat_n, 1, 1, 1),
                                emb3[i : i + 1].repeat(repeat_n, 1, 1, 1),
                                emb4[i : i + 1].repeat(repeat_n, 1, 1, 1),
                            ])

                        emb_out = torch.cat(pieces, dim=0)  # This creates a sequence of 25 frames for input image conditioning
